  ...
  useEffect(() => {
    async function loadStudentResults() {
      ...
      const { data } = await supabase
        .from("submissions")
        .select(`
          id, total_marks, status, feedback, evaluated_at, is_released_by_teacher,
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted mt-1">Review graded records, automated point allocations, and AI insight critiques.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4 animate-slide-up">
          {submissions.map((sub) => {
            const scoresReleased = sub.is_released_by_teacher;
            return (
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right">
                      ...
                      <p className="text-lg font-black text-primary font-mono">
                        {scoresReleased ? `${sub.total_marks} / ${sub.exams?.total_marks}m` : "Awaiting Faculty Release"}
                      </p>
                    </div>
                    
                    <span className={`text-[10px] px-2.5 py-1 border rounded-md font-bold uppercase tracking-wider ${
                      scoresReleased ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                    }`}>{scoresReleased ? "Evaluated" : "Locked"}</span>
                  </div>
                </div>

                {scoresReleased && sub.feedback ? (
                  <div className="p-3.5 bg-secondary/40 border border-border rounded-xl text-xs text-muted leading-relaxed">
                    <span className="font-bold text-foreground block mb-1 flex items-center gap-1"><Sparkles size={12} className="text-primary"/> AI Evaluation Insight Pipeline:</span>
                    {sub.feedback}
                  </div>
                ) : (
                  !scoresReleased && (
                    <p className="text-xs italic text-muted">Evaluation matrix processing finalized. Metrics will show once released by your instructor.</p>
                  )
                )}
              </div>
            );
