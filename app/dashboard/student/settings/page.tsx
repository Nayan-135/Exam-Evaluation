...
export default function StudentSettingsPage() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      setMsg(error ? "Profile modification error." : "Personal credentials successfully updated.");
    }
    setSaving(false);
  };
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto space-y-8 animate-fade-in">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Workstation Settings</h1>
        <p className="text-sm text-muted mt-1">Manage system configurations, personal profiles, and verification accounts.</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-2xs">
        <div className="p-4 border-b border-border bg-secondary/30 flex items-center gap-2 font-bold text-sm text-foreground">
          <User size={16} className="text-primary"/> Personal Core Profile
        </div>
        ...
        <form onSubmit={handleUpdate} className="p-6 space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-muted uppercase">Registered Student Email</label>
